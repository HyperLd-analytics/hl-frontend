  ...
  return (
    <main className="flex min-h-screen items-center justify-center px-4">
      <Card className="w-full max-w-sm space-y-4">
        <h1 className="text-xl font-semibold">{t("login")} Hyperliquid Lens</h1>
        <p className="text-sm text-muted-foreground">
          先用账号密码登录，后续可扩展 OAuth 或钱包签名。
        </p>
        ...
        {error && <PageError title={t("loginFailed")} message={error.message} />}
        ...
        <div className="grid grid-cols-2 gap-3">
          <Button
            ...
            onClick={async () => {
              try {
                const res = await request<{ authUrl: string; state: string }>({
                  path: "/auth/google",
                  method: "GET",
                });
...
export default function LoginPage() {
  const tCommon = useTranslations("common");
  return (
    <Suspense fallback={<main className="flex min-h-screen items-center justify-center px-4">{tCommon("loading")}</main>}>
      <LoginFormInner />
    </Suspense>
  );
}
