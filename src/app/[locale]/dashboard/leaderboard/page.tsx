  ...
  return (
    <div className="space-y-4">
      ...
      <Card className="flex items-center gap-2 p-3">
        ...
        >
          {exporting ? tCommon("loading") : tCommon("export")}
        </Button>
      </Card>
      ...
      <Card className="overflow-x-auto">
        <table className="w-full text-sm">
          <thead className="text-muted-foreground">
            <tr>
              <th className="px-3 py-2 text-left">{t("address")}</th>
              <th className="px-3 py-2 text-left">{t("score")}</th>
              <th className="px-3 py-2 text-left">{t("totalPnl")}</th>
              <th className="px-3 py-2 text-left">{t("winRate")}</th>
            ...
            {(data?.items ?? []).map((item) => (
              <tr key={item.address} className="border-t border-border hover:bg-muted/50">
                <td className="px-3 py-2">
                  <a
                    href={`/${locale}/dashboard/wallet/${item.address}`}
        ...
        {(!data?.items || data.items.length === 0) && !loading && (
          <div className="py-12 text-center text-muted-foreground">{tCommon("noData")}</div>
        )}
      </Card>
